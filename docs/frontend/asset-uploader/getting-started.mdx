...
素材上传组件链接是：`{authUrl}?aesUserInfo={encodeURIComponent(经过 aesKey 加密的用户信息)}&targetUrl={encodeURIComponent({domain}/dam_enterprise/asset-uploader)}`。参照<Link to="/docs/frontend/common/authorize">授权</Link>。
