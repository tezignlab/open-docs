...
tezignSdk.use([clickPlugin(clickPluginsOptions), pageViewPlugin(pvPluginsOptions), persistLocalStoragePlugin(persistLocalStoragePluginOptions)])
```
### 浏览器引入
```typescript
// 插件：
 const {pageViewPlugin, clickPlugin, persistLocalStoragePlugin} = window.tezignExternalTrack.plugin

window.tezignExternalTrack.use([clickPlugin(clickPluginsOptions), pageViewPlugin(pvPluginsOptions), persistLocalStoragePlugin(persistLocalStoragePluginOptions)])
